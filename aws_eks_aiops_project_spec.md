...
K8sGPT Pod / CronJob
→ 掃描 EKS Cluster
→ Webhook 發送至 API Gateway
→ 觸發 Lambda
→ 搶佔 DynamoDB 原子鎖 (防並發重複調用與告警風暴)
→ 送至 AWS Bedrock (經 VPC Endpoint 全內網通訊)
→ Bedrock 產生報告與 Action JSON
→ Lambda 進行防注入檢驗與白名單校驗
→ 存入一次性 Token 與安全指令至 DynamoDB (狀態改為 PENDING)
→ SNS Email / LINE 發送帶 Token 的審批連結
→ 工程師點擊 Approve 連結
→ Lambda 驗證 Token 且狀態為 PENDING
→ 更新 DynamoDB 狀態為 APPROVED (防範二次點擊)
→ Lambda 啟動 CodeBuild (於私有子網 VPC 執行)
→ CodeBuild 下載 v1.34.0 kubectl 並進行 SHA256 校驗
→ CodeBuild 在內網套用修正指令 (不使用 eval)
→ 發送修復結果通知
```
...
### 18.4 告警去重、冷卻與安全審批機制 (DynamoDB Cache)

為避免因 Pod 連續崩潰導致維運通道在短時間內遭灌爆（告警風暴），並確保審批流程的高安全性，我們在 Lambda 流程中設計了以下機制：

1. **原子寫入鎖 (Atomic Lock)**：使用一張輕量型的 DynamoDB 表（`eks-aiops-demo-alert-cache`），以 `AlertHash`（結合 Namespace/ResourceName/ErrorMsg 的 SHA256）為主鍵。Webhook 觸發時，Lambda 使用 DynamoDB 條件寫入（`ConditionExpression`）搶鎖：
   - 若 `AlertHash` 存在且未過期（冷卻期為 10 分鐘，由 ExpiryTime 判定），寫入失敗，立即攔截並過濾告警，避免在 Bedrock 運算期間重複發送。
   - 若不存在或已過期，則以 `Status = ANALYZING` 鎖定該告警。
2. **安全一次性 Token (One-Time Token)**：Lambda 在搶鎖成功後，生成一個隨機的 UUID 作為 `OneTimeToken` 寫入 DynamoDB，並組裝於 Approve / Reject 郵件連結中。
3. **狀態機防重複點擊 (Double-Click Prevention)**：工程師點擊 Approve 連結後，Lambda 會比對網址傳遞的 Token 且嚴格校驗資料庫內狀態必須為 `PENDING`，驗證成功後立即將狀態變更為 `APPROVED` 才會觸發 CodeBuild，徹底防止連結二次點擊或越權觸發。
